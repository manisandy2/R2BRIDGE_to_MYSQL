...
#                     parsed_date = None
#
#                     # Accept both datetime and string values
...
#                                 continue
#
#                     # Convert successfully parsed values to date only
#                     if parsed_date:
#                         converted_row[field.name] =  parsed_date if isinstance(parsed_date, datetime) else datetime.combine(parsed_date, datetime.min.time())
#                     else:
#                         print(f"⚠️ Row {row_idx}: Unrecognized date format in '{field.name}': {val}")
#                         converted_row[field.name] = None
#
#                 # --- Default: strings or other types ---
#                 else:
...
#                     except Exception as e:
#                         print(f"⚠️ Error converting {date_field}: {e}")
...
#     arrow_table = pa.Table.from_pylist(arrow_tables, schema=arrow_schema)
#     print("Arrow table ....")
#     print(arrow_table.schema)
#     print("Combined Table:",arrow_table)
#     arrow_end = time.time()
#     print(f"Arrow conversion completed in {arrow_end - arrow_start:.2f} sec (rows={arrow_table.num_rows})")
#
#     # -------------------------------------------------
...
def insert_transaction_phone_data(
    ...
    try:
        ...
        #                 row[f] = 0.0
        #
        #     # 2️⃣ Convert text mobile numbers → int64
        #     if isinstance(row.get("customer_mobile__c"), str):
        #         try:
        #             row["customer_mobile__c"] = int(row["customer_mobile__c"])
        #         except ValueError:
        #             row["customer_mobile__c"] = None
        #
        #     # 3️⃣ Convert Item_Code__c to int64
        #     if isinstance(row.get("Item_Code__c"), str):
        #         try:
        #             row["Item_Code__c"] = int(row["Item_Code__c"])
        #         except ValueError:
        #             row["Item_Code__c"] = None
        #
        #
        #     for date_field in ["Bill_Date__c", "Invoice_Date__c", "CreatedDate"]:
        #         val = row.get(date_field)
        ...
        for row in rows:
            ...
            # 2️⃣ Convert mobile numbers to int64
            ...
            # 4️⃣ Convert date strings to Python `date` object (yyyy-mm-dd only)
            for date_field in ["Bill_Date__c", "Invoice_Date__c", "CreatedDate"]:
                val = row.get(date_field)
                if val in ("", " ", None):
                    row[date_field] = None
    ...
    # print("Arrow Table Schema:", combined_table.schema)
    # print("Total Rows in Combined Table:", combined_table.num_rows)
