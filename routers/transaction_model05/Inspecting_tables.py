    ...
    try:
        ...
        for snapshot in table.snapshots():
            snapshots.append({
                ...
                "timestamp": datetime.fromtimestamp(snapshot.timestamp_millis / 1000).isoformat(),
                "operation": snapshot.summary.get("operation"),
                "manifest_list": snapshot.manifest_list,
                "added_files": snapshot.summary.get("added-data-files"),
                "deleted_files": snapshot.summary.get("deleted-data-files"),
                "total_files": snapshot.summary.get("total-data-files"),
                "added_records": snapshot.summary.get("added-records"),
                "deleted_records": snapshot.summary.get("deleted-records"),
...
@router.get("/iceberg/inspect-partitions")
...
@router.get("/iceberg/inspect-refs")
def inspect_iceberg_refs(
    ...
    try:
        ...
        refs_info = []
        refs = table.refs  # ✅ PyIceberg exposes refs as a dict-like structure

        print("refs",refs.items())
        for ref_name, ref in refs.items():
            refs_info.append({
                "name": ref_name,
                "type": ref.type,  # "branch" or "tag"
...
@router.get("/iceberg/inspect-manifests")
...
@router.get("/iceberg/inspect-history")
...
@router.get("/iceberg/inspect-files")
...
@router.get("/iceberg/table-total/count")
